jQuery(document).ready(function($) {
  ...
  $.post({url: `/cart/cartItems`, success: function(result){
    displayCartQuantity(result);
  }});
  $('button.add-cart').click(function(){
    adjustSession($(this).attr("data-id"),1,(n_item)=>{
...
function adjustSession(proID, quantity, callback){
  $.post({url: `/cart/add/${proID}/${quantity}`, success: function(result){
    callback(result);
  }});
}
